...
| MPPI-006 | Milestone 5/6A solve-time profiling | Milestone 5 mean solve time approximately 1.1-1.23 s; Milestone 6A default-target mean solve time approximately 0.133-0.139 s against a 0.10 s period with 100% deadline overrun | Runtime profile separating sampling, model rollout, cost computation, weighting, ROS overhead, metrics, and allocation cost | Profiling report from the current Ubuntu environment using trajectory and cylinder-navigation launch paths | Controller | High |
| MPPI-007 | Python rollout optimization | Sequential Python rollout over samples and horizon | Optimization plan and benchmark | Profile-guided implementation plan for vectorization, caching, multiprocessing, compiled kernels, or reduced rollout load without changing controller semantics | Controller | High |
...
| EVAL-002 | Cross-process ROS_DOMAIN_ID reservation | `ctr_run_evaluation` selects valid distinct domains per paired run, but no deterministic interprocess reservation or locking mechanism is implemented | Domain reservation policy | Add lock-file, registry, or equivalent collision-resistant domain allocation and concurrent-run tests | Evaluation | Medium |
