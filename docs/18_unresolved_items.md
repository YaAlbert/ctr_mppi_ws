...
|---|---|---|---|---|---|
| BUILD-001 | Clean Ubuntu build result exists through committed Milestone 5D.1 before the final Python-only path fix | A clean isolated 11-package build passed before the final experiment-group/path-containment fix; the final fix changed only `run_evaluation.py` and `test_run_evaluation.py`; focused tests passed after that fix | Re-run the clean isolated build after future source or package changes, or before release | Build | Partially resolved for current source |
...
| MPPI-006 | Milestone 5 solve-time profiling | Mean solve time approximately 1.1-1.23 s, maximum approximately 1.19-1.54 s | Runtime profile separating sampling, model rollout, cost computation, weighting, ROS overhead, metrics, and allocation cost | Profiling report from the current Ubuntu environment using the Milestone 5 trajectory launch path | Controller | High |
