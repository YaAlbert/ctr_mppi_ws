...
| Milestone 5: tip trajectory tracking | Functionally integrated and runtime smoke verified; performance not verified | Focused tests passed: `ctr_mppi_controller` 77, `ctr_bringup` 25, `ctr_sim` 4, total 106; clean isolated build finished all 11 packages; circle, ellipse, and helix each ran for 12 seconds; `/ctr/reference/path`, `/ctr/reference/horizon`, `/ctr/reference/tip`, and `/ctr/controller/trajectory_metrics` published; commands were finite and within configured limits; no hardware node started; launch exited with code 0; no residual project process or zombie remained | Real-time trajectory control; timestamp-aligned metrics; rigorous zero-command baseline synchronization; stronger nontrivial trajectory experiments; MPPI solve-time profiling; rollout optimization; complete MPPI formulation review; physical tracking validation; hardware validation |
...
| METRIC-001 | Timestamp-aligned trajectory metrics | Runtime smoke metrics are not strongly synchronized across state, reference, command, and diagnostic timestamps | Time-alignment design | Metrics update that records or interpolates state/reference/command samples against consistent timestamps before claiming tracking-performance results | Evaluation | High |
| EVAL-001 | Rigorous zero-command baseline synchronization | Baseline comparisons are smoke-test evidence only | Repeatable experiment protocol | Baseline harness ensuring identical initial state, trajectory start time, warm-up interval, sampling window, metric reset, and observer alignment | Evaluation | High |
